...
for field_name, field in field_group.items():
    ...
    for instance in instances:
        ...
        if type in ["Integer", "Date", "Text", "Continuous", "Time"]:
            ...
            start = 0
            if f"f.{field}.{instance}.0" not in data:
            ...
            for index in entries:
                ...
                try:
                    col = data[f"f.{field}.{instance}.{index + start}"].copy()
                except KeyError:
            ...
            if len(set(labels)) == len(labels):
                column = pandas.Categorical(data[f"f.{field}.{instance}.0"], categories)
                ...
                column = data[f"f.{field}.{instance}.0"]
            ...
            for index in entries:
                try:
                    col = data[f"f.{field}.{instance}.{index+start}"]
                except KeyError:
                    print(f"WARNING: failed to find field {field_name}:{field}.{index} in data. Skipping.")
                    continue
        ...
        if instance == 0:
            base_output.update(output)
        else:
            instance_output = {key + f"_V{instance}":value for key, value in output.items()}
            repeat_output.update(instance_output)

# Output the base (first repeat) data
output = pandas.DataFrame(base_output)
if args.output.endswith("txt"):
    output.to_csv(args.output, sep="\t")
elif args.output.endswith("h5"):
    output.to_hdf(args.output, key="data", mode="w", format="table")
