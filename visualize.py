...
def visualize(filename, show=True, M10_view=False, **kwargs):
    ...
    if not M10_view:
        ...
        painters.append(matplotlib.lines.Line2D([0],[0], color="k", label="accel"))
        fig.legend(painters, ['M10', 'L5', 'accel'])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--M10_view", required=False, default=False, action="store_const", const=True, dest="M10_view")
    args = parser.parse_args()

    for filename in args.filename:
        data, results = visualize(filename, M10_view=args.M10_view)
