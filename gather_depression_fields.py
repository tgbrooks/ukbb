...
mental_health_fields = dict(
    ...
    sought_profession_help_mental_distress = 20499,
...
anxiety_dependent_fields = dict(
    ever_felt_worried_more_than_month = 20421, #NOTE: dependent upon 20420 but not 20425
    ...
    profession_informed_anxiety = 20428,
...
depression_dependent_fields = dict(
    ...
    fraction_of_day_worst_epsidode = 20436,
    frequency_depressed_days_worst_episode = 20439,
    import_on_normal_roles_worst_episode = 20440,
    number_depressed_periods = 20442,
    profession_informed_about_depression = 20448,
    substances_taken_for_depression = 20546,
    thoughts_of_death_worst_epsiode = 20437,
    weight_change_worst_epsiode = 20536,
)

# These were only asked if sleep_change_worst_episode
...
mania_dependent_fields = dict(
    ...
    manifestations_of_mania = 20548, # many categories
    severity_of_problems_due_to_mania = 20493,
)

# Collect all the different fields
field_groups = [general_fields, mental_health_fields, anxiety_dependent_fields, recent_anxiety, depression_dependent_fields, sleep_change_type_fields, recent_depression]
all_fields = dict()
for group in field_groups:
    all_fields.update(group)

invert_fields = {val:key for key, val in fields.items()}
